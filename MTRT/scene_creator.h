...
public:
	...
	static void CreateCornellBox(std::vector<std::shared_ptr<Hitable>>& obj_list, std::vector<std::shared_ptr<Hitable>>& important_list){
		...
		std::shared_ptr<Material> aluminum(new Metal(Vec3(0.8, 0.85, 0.88), 0.1));
		...
		////sphere
		auto boundary = std::make_shared<Translate>(new Sphere(Vec3(0, 0, 0), 100, aluminum), Vec3(400, 100, 150));

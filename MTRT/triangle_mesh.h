...
const std::string MODEL_PATH = "resources/objs/awp_dragon.obj";
class TriangleMesh :public Hitable{
public:
	TriangleMesh() {}
	TriangleMesh(const std::string& obj_file_path,Material* mat_ptr) {
		std::vector<Vertex> tmp_vertices;
		std::vector<uint32_t> tmp_indices;
		std::vector<Hitable *> triangle_arry;
		LoadModel(obj_file_path, triangle_arry, mat_ptr);
		for (int i = 0; i < tmp_indices.size(); i += 3) {
			Triangle * triangle_ptr = new Triangle(tmp_vertices[tmp_indices[i]], tmp_vertices[tmp_indices[i + 1]], tmp_vertices[tmp_indices[i + 2]], mat_ptr);
	...
	void LoadModel(const std::string& obj_file_path,std::vector<Hitable*>& hitable_array, Material* mat_ptr);
	BVHNode * root_ptr_;
};


void TriangleMesh::LoadModel(const std::string& obj_file_path,std::vector<Hitable*>& hitable_array, Material* mat_ptr) {
	...
	for (const auto& shape : shapes) {
		...
		for (int i = 0; i < shape.mesh.indices.size();i+= 3) {
			...
			hitable_array.push_back(new Triangle(vertex[0], vertex[1], vertex[2], mat_ptr));
		}
	}
